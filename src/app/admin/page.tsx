...
export default function AdminPage() {
  ...
  const handleGeminiImport = (extracted: Shift[], mode: "replace" | "merge") => {
    const existing = schedule?.shifts ?? [];
    let next: Shift[];
    if (mode === "replace") {
      next = [
        ...existing.filter((s) => !s.date.startsWith(monthKey)),
        ...extracted,
      ];
    } else {
      const extractedKeys = new Set(extracted.map((s) => `${s.doctorId}|${s.date}`));
      next = [
        ...existing.filter((s) => !extractedKeys.has(`${s.doctorId}|${s.date}`)),
        ...extracted,
      ];
    }
    persistShifts(next);
  ...
  const GITHUB_DATA_UPLOAD = "https://github.com/iamarasinghe96/awhroster/upload/main/public/data";
  ...
  const handlePublishHolidays = () => {
    exportHolidaysJSON(holidays);
    setTimeout(() => window.open(GITHUB_DATA_UPLOAD, "_blank"), 400);
  };
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-slate-800 text-white px-4 py-3 flex items-center justify-between flex-wrap gap-3 z-40 sticky top-0">
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="text-slate-400 hover:text-white text-xs border border-slate-600 hover:border-slate-400 rounded px-2 py-1 transition">
            Back to Doctor View
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span className={`text-xs px-2 py-1 rounded-full transition ${
            saveStatus === "saving" ? "bg-amber-700 text-amber-100"
            : saveStatus === "saved" ? "bg-emerald-700 text-emerald-100"
            : "hidden"
          }`}>
            {saveStatus === "saving" ? "Saving..." : "Saved locally"}
          </span>

          <button onClick={() => setShowGeminiImport(true)}
          ...
          <button onClick={() => setShowHolidayKey(!showHolidayKey)}
            className="text-xs px-2 py-1 rounded bg-slate-700 hover:bg-slate-600 text-green-300 transition">PH Key</button>

          <button onClick={handleExportCSV}
            className="text-xs px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium transition">CSV</button>
        </div>
      </header>
          ...
          <div className="flex items-start justify-between gap-6 max-w-4xl flex-wrap">
            <div className="flex-1 min-w-[240px]">
              ...
              <ol className="list-decimal list-inside space-y-1 text-blue-200 text-xs">
                <li>Click the publish button - it downloads the file and opens GitHub</li>
                <li>Drag and drop the downloaded file onto the GitHub page and commit</li>
                <li>GitHub Actions rebuilds automatically - doctors refresh to see changes</li>
              </ol>
            </div>
            <div className="flex flex-col gap-2 flex-shrink-0">
              <button onClick={handlePublishSchedule} disabled={!schedule}
              ...
              <button onClick={handlePublishDoctors}
      ...
      {showHolidayKey && (
        <div className="bg-green-50 border-b border-green-200 px-4 py-3">
          <h3 className="text-xs font-bold text-green-800 mb-2">Loaded Public Holidays ({holidays.length})</h3>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          {loading ? (
            <div className="flex-1 flex items-center justify-center text-slate-500">
              <div className="text-center"><div className="text-2xl mb-2">Loading</div>Loading schedule...</div>
            </div>
          ) : (
            <AdminCalendar
      ...
      <footer className="bg-white border-t border-slate-200 px-4 py-2 flex flex-wrap gap-3 items-center text-xs text-slate-500 z-10">
        ...
        <span className="ml-auto text-slate-400">Changes saved locally. Use Deploy to publish.</span>
      </footer>

      {showAddDoctor && <AddDoctorModal onClose={() => setShowAddDoctor(false)} onAdd={handleAddDoctor} />}
      {editingDoctor && (
        <AddDoctorModal title={`Edit - ${editingDoctor.name}`} initialValues={editingDoctor}
          onClose={() => setEditingDoctor(null)} onAdd={handleEditDoctor} />
      )}
      {showGeminiImport && (
        <GeminiImportModal
          doctors={doctors}
