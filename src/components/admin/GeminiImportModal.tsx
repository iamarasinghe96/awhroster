...
- Skip anyone who doesn't match a known doctor
...
- For "type" use exactly one of:
    ...
    "off"            scheduled day off  (use startTime/endTime "00:00")
    "request_off"    requested day off  (use startTime/endTime "00:00")
    "public_holiday" public holiday     (use startTime/endTime "00:00")
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const valid = shifts.filter(
        (s) => validDoctorIds.has(s.doctorId) && typeof s.date === "string" && s.date.startsWith(monthKey)
      );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200">
          <div>
            <h2 className="text-base font-bold text-slate-800">Import from PDF - Gemini AI</h2>
            <p className="text-xs text-slate-500 mt-0.5">
              Upload the roster PDF and Gemini will extract shifts for{" "}
              <strong>{format(parseISO(monthKey + "-01"), "MMMM yyyy")}</strong>
            </p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  ...
                  <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer"
                    className="ml-2 text-blue-600 font-normal hover:underline">Get a key</a>
                </label>
                ...
                <label className="flex items-center gap-2 mt-1.5 text-xs text-slate-500 cursor-pointer">
                  <input type="checkbox" checked={saveKey} onChange={(e) => setSaveKey(e.target.checked)} className="rounded" />
                ...
                <div onClick={() => fileRef.current?.click()}
                  ...
                  {pdfFile ? (
                    ...
                    <>
                      ...
                      <div className="text-sm text-slate-500">Click to select PDF</div>
                    </>
                  )}
                </div>
          ...
          {step === "loading" && (
            <div className="flex flex-col items-center justify-center py-12 gap-3 text-slate-500">
              ...
              <p className="text-sm">Gemini is reading the PDF...</p>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <label className="flex items-center gap-1.5 cursor-pointer">
                    ...
                    <span className="text-slate-400">(clears {existingShifts.filter(s => s.date.startsWith(monthKey)).length} existing)</span>
                  </label>
                  <label className="flex items-center gap-1.5 cursor-pointer">
          ...
          <div className="flex gap-2">
            {step === "preview" && (
              <button onClick={() => setStep("input")}
                className="px-4 py-2 text-sm border border-slate-300 rounded-lg hover:bg-slate-50 transition">Back</button>
            )}
            {(step === "input" || step === "error") && (
              <button onClick={handleExtract} disabled={!pdfFile || !apiKey.trim()}
                className="px-5 py-2 text-sm bg-blue-600 hover:bg-blue-700 disabled:opacity-40 text-white rounded-lg font-medium transition">
                Extract with Gemini
              </button>
            )}
            {step === "preview" && (
              <button onClick={handleConfirm}
                className="px-5 py-2 text-sm bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg font-medium transition">
