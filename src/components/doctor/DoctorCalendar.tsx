      ...
      {otherDoctors && otherDoctors.length > 0 && (
        <div className="mt-1 pt-1 border-t border-current border-opacity-20">
          {otherDoctors.slice(0, 2).map(({ doctor, shift: s }) => (
            ...
            >
              {doctor.name.split(" ")[0]} ({s.startTime}–{s.endTime})
            </div>
          ))}
          {otherDoctors.length > 2 && (
            <div className="text-[10px] opacity-50">+{otherDoctors.length - 2} more</div>
          )}
        </div>
      )}
    </div>
                ...
                return (
                  ...
                  >
                    ...
                    <ShiftCell
                      ...
                      otherDoctors={inCurrentMonth ? undefined : others}
                    />
                  </div>
                );
              })}
